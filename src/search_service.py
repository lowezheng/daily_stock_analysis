...
class AkShareNewsProvider(BaseSearchProvider):
    """
    AkShare News Provider (Free, No API Key Required)

    Features:
    - Individual stock news from East Money (ak.stock_news_em)
    - Completely free, no API key needed
    - Chinese-friendly, real-time updates

    Docs: https://akshare.akfamily.xyz/data/stock/stock.html#id219
    """
    ...
    def search(self, query: str, max_results: int = 5, days: int = 7) -> SearchResponse:
        """
        Execute search (override to skip API key check)

        AkShare doesn't require API key, so we skip the base class check.
        """
        start_time = time.time()
        try:
            ...
            elapsed = time.time() - start_time
            logger.error(f"[{self.name}] Search '{query}' failed: {e}")
            return SearchResponse(
                query=query,
                results=[],
                provider=self.name,
    ...
    def _do_search(
        ...
        try:
            ...
            if not stock_code:
                logger.warning(
                    f"[AkShare] Cannot extract stock code from query: {query}"
                )
                return SearchResponse(
                    query=query,
                    results=[],
                    provider=self.name,
                    success=False,
                    error_message="Cannot recognize stock code",
                )

            logger.info(f"[AkShare] Fetching stock news: {stock_code}")

            df = ak.stock_news_em(symbol=stock_code)

            if df is None or df.empty:
                logger.warning(f"[AkShare] No news found for {stock_code}")
                return SearchResponse(
                    query=query,
                    results=[],
                    provider=self.name,
                    success=False,
                    error_message="No related news found",
                )

            logger.info(f"[AkShare] Retrieved {len(df)} news items")
            ...
            for _, row in df.head(max_results).iterrows():
                ...
                results.append(
                    SearchResult(
                        ...
                        snippet=str(row.get("新闻内容", row.get("内容", "")))[:500],
                        url=str(row.get("新闻链接", row.get("链接", ""))),
                        source=str(row.get("文章来源", row.get("来源", "East Money"))),
            ...
            error_msg = str(e)
            logger.error(f"[AkShare] Failed to fetch news: {error_msg}")
            return SearchResponse(
                query=query,
                results=[],
                provider=self.name,
    ...
    def _extract_stock_code(query: str) -> Optional[str]:
        """
        Extract stock code from query string

        Supported formats:
        - A-share: 6 digits (000001, 600519)
        - HK stock: hk + 5 digits (hk00700) or 5 digits
        - US stock: 1-5 letters (AAPL, TSLA)
        """
        import re

        a_stock = re.search(r"\b(\d{6})\b", query)
        if a_stock:
            return a_stock.group(1)

        hk_stock = re.search(r"\b(hk)?(\d{5})(\.HK)?\b", query, re.IGNORECASE)
        if hk_stock:
            return hk_stock.group(2)

        us_stock = re.search(r"\b([A-Z]{1,5})(\.[A-Z])?\b", query)
        if us_stock:
            return us_stock.group(1)

        return None


class AkShareFlashProvider(BaseSearchProvider):
    """
    AkShare Financial Flash Provider (Free, No API Key Required)

    Features:
    - Sina global financial flash news (ak.stock_info_global_sina)
    - Completely free, real-time updates
    - Global financial news coverage

    Docs: https://akshare.akfamily.xyz/data/stock/stock.html#id78
    """
    ...
    def _do_search(
        ...
        """Execute AkShare flash news search with keyword filtering"""
        try:
            import akshare as ak
        except ImportError:
            return SearchResponse(
        ...
        try:
            ...
            if (
                self._flash_cache["data"] is not None
                and current_time - self._flash_cache["timestamp"]
                < self._flash_cache["ttl"]
            ):
                df = self._flash_cache["data"]
                logger.debug(f"[AkShare-Flash] Using cached data")
            else:
                logger.info(f"[AkShare-Flash] Fetching Sina financial flash news...")
                df = ak.stock_info_global_sina()

                if df is not None and not df.empty:
                    self._flash_cache["data"] = df
                    self._flash_cache["timestamp"] = current_time
                    logger.info(f"[AkShare-Flash] Retrieved {len(df)} flash news items")
            ...
            if df is None or df.empty:
                return SearchResponse(
                    ...
                    provider=self.name,
                    success=False,
                    error_message="No flash news data retrieved",
                )
            ...
            if not keywords:
                ...
                mask = df.apply(lambda row: self._match_keywords(row, keywords), axis=1)
                filtered_df = df[mask].head(max_results)

            if filtered_df.empty:
                logger.warning(f"[AkShare-Flash] No keywords matched: {keywords}")
                return SearchResponse(
                    query=query,
                    results=[],
                    provider=self.name,
                    success=False,
                    error_message="No related flash news found",
                )

            results = []
            for _, row in filtered_df.iterrows():
                content = str(row.get("内容", ""))
                time_str = str(row.get("时间", ""))[:16] if "时间" in row else ""

                # Flash news has no title, use first 50 chars of content as title
                ...
                results.append(
                    SearchResult(
                        ...
                        url="",
                        source="Sina Finance",
                        published_date=time_str if time_str else None,
                    )
                )
            ...
            error_msg = str(e)
            logger.error(f"[AkShare-Flash] Failed to fetch flash news: {error_msg}")
            return SearchResponse(
                query=query,
                results=[],
                provider=self.name,
    ...
    def _extract_keywords(query: str) -> List[str]:
        """Extract keywords from query (filter out stock codes and common words)"""
        import re

        query = re.sub(r"\b\d{6}\b", "", query)
        query = re.sub(r"\b[hH][kK]?\d{5}\b", "", query)
        query = re.sub(r"\b[A-Z]{1,5}\b", "", query)
    ...
    @staticmethod
    def _match_keywords(row, keywords: List[str]) -> bool:
...
class SearchService:
    ...
    def __init__(
        ...
        Args:
            ...
            use_akshare: 是否启用 AkShare 免费新闻接口（默认启用）
        """
        self._providers: List[BaseSearchProvider] = []
        self.news_max_age_days = max(1, news_max_age_days)

        # 1. AkShare (Free, No API Key, Highest Priority)
        if use_akshare:
            akshare_news = AkShareNewsProvider()
            akshare_flash = AkShareFlashProvider()
            if akshare_news.is_available:
                self._providers.append(akshare_news)
                self._providers.append(akshare_flash)
                logger.info("已配置 AkShare 新闻（免费）")

        # 2. Bocha (Chinese search optimization, AI summary)
        if bocha_keys:
            self._providers.append(BochaSearchProvider(bocha_keys))
            logger.info(f"已配置 Bocha 搜索，共 {len(bocha_keys)} 个 API Key")

        # 3. Tavily (More free quota, 1000/month)
        if tavily_keys:
            self._providers.append(TavilySearchProvider(tavily_keys))
            logger.info(f"已配置 Tavily 搜索，共 {len(tavily_keys)} 个 API Key")

        # 4. Brave Search (Privacy-first, global coverage)
        if brave_keys:
            self._providers.append(BraveSearchProvider(brave_keys))
            logger.info(f"已配置 Brave 搜索，共 {len(brave_keys)} 个 API Key")

        # 5. SerpAPI as fallback (100/month)
        if serpapi_keys:
    ...
    def search_stock_news(
        ...
        logger.info(
            f"搜索股票新闻: {stock_name}({stock_code}), query='{query}', 时间范围: 近{search_days}天"
        )
        ...
        # A-share priority: Use AkShare first (free, accurate)
        if not is_foreign and stock_code.isdigit() and len(stock_code) == 6:
            for provider in self._providers:
                if provider.name == "AkShare" and provider.is_available:
                    response = provider.search(query, max_results, days=search_days)
                    if response.success and response.results:
                        logger.info(f"使用 AkShare 搜索成功（A股优先）")
                        self._put_cache(cache_key, response)
                        return response
                    break  # AkShare failed, continue to other providers
        ...
        for provider in self._providers:
            ...
            # Skip AkShare if already tried (A-share case)
            if not is_foreign and stock_code.isdigit() and len(stock_code) == 6:
