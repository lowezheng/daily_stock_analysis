...
class Config:
    ...
    analysis_strategy: str = (
        "trend"  # Strategy name: trend, value, swing, or custom/{name}
    )
    analysis_strategy_path: Optional[str] = (
        None  # Direct path to strategy file (takes priority)
    )
    ...
    def _load_from_env(cls) -> "Config":
        ...
        return cls(
            ...
            analysis_strategy=os.getenv("ANALYSIS_STRATEGY", "trend"),
            analysis_strategy_path=os.getenv("ANALYSIS_STRATEGY_PATH") or None,
            ...
            schedule_run_immediately=os.getenv(
    ...
    def validate(self) -> List[str]:
        ...
        if (
            ...
            warnings.append(
                "警告：未配置 Gemini/Anthropic/OpenAI API Key，AI 分析功能将不可用"
            )
        elif not self.gemini_api_key and not self.anthropic_api_key:
            warnings.append(
                "提示：未配置 Gemini/Anthropic API Key，将使用 OpenAI 兼容 API"
            )
        ...
        if (
            ...
            warnings.append(
                "提示：未配置搜索引擎 API Key (Bocha/Tavily/Brave/SerpAPI)，新闻搜索功能将不可用"
            )
