...
from src.core.prompt_loader import load_strategy_prompt
...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Load strategy prompt from external file
        self.system_prompt = load_strategy_prompt(
            strategy_name=config.analysis_strategy,
            strategy_path=config.analysis_strategy_path,
        )
        logger.info(
            f"[PromptLoader] Loaded strategy: {config.analysis_strategy or 'default'}"
        )
    ...
    def _call_openai_api(self, prompt: str, generation_config: dict) -> str:
        ...
        def _is_unsupported_param_error(error_message: str, param_name: str) -> bool:
            ...
            return (
                "400" in lower_msg
                or "unsupported parameter" in lower_msg
                or "unsupported param" in lower_msg
            ...
            try:
                ...
                try:
                    response = self._openai_client.chat.completions.create(
                        **_kwargs_with_mode(mode)
                    )
                except Exception as e:
                    error_str = str(e)
                    if mode == "max_tokens" and _is_unsupported_param_error(
                        error_str, "max_tokens"
                    ):
                        ...
                        mode == "max_completion_tokens"
                        and _is_unsupported_param_error(
